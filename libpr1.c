...
#else
						 char * lib_output_color(name, color, ka, kd, ks, shine, ang, kt, i_of_r)
						 char *name;
COORD3 color;
...
#endif
{
	...
	case OUTPUT_RIB:
		...
		fprintf(gOutfile, "Surface \"spd\" \"Ka\" %#g \"Kd\" %#g" 
			...
			ka, kd, shine, 1.0/phong_pow, ks, kt, i_of_r);
		break;
	...
	case OUTPUT_VRML1:
		...
		*/
		...
		if (shine != 0) {
			...
			tab_indent();
			fprintf(gOutfile, "specularColor %g %g %g\n",
				(float)(color[X]*shine), (float)(color[Y]*shine), (float)(color[Z]*shine));
			
			tab_indent();
			fprintf(gOutfile, "shininess %g\n", (phong_pow > 128.0) ? 1.0f : (float)(phong_pow / 128.0));
		} else {
			tab_indent();
			fprintf(gOutfile, "diffuseColor %g %g %g\n",
				(float)(color[X]*kd), (float)(color[Y]*kd), (float)(color[Z]*kd));
